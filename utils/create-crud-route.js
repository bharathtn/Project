...
module.exports = function (modelName, fieldSelection, volatileFields, requiredFields) {
    ...
    router.post('/', function (req, res) {
        ...
        var processManyRequest = function () {
            //Ensure all required fields are present
            var valid = q.reduce(function (o, q) {
                return o && requiredFields.reduce(function (m, d) {
                    return (q[d] && m);
                }, true);
            }, true)
            if (!(q && valid)) {
            ...
            Model.create(q, function (err, entities) {
                if (!err) {
                    var d = {};
                    d[pluralize(modelName)] = entities.map(function (entity) {
                        return pick(entity, fieldSelection)
                    });
                    res.status(201).json(d);
                } else if (err.code == 11000) {
                    res.boom.conflict('Model with same text already exists!');
        ...
        var q = req.body[modelName] || req.body[pluralize(modelName)];
    ...
    /**
     * GET /entities/:id/:id
     * returns an entity as specified by :id :id
     */
    router.get('/:qid/:uid', function (req, res) {
        ...
        Model.find({
            ...
            if (!err && (entity.length != 0)) {
                var d = {};
                d[modelName] = entity[0].toObject();
                res.json(d);
            } else if (entity.length == 0) {
                console.error("GET /" + modelName + '/' + req.params.qid + '/' + req.params.uid, err);
                res.boom.notFound('No matching IDs');
            } else {
                res.boom.badImplementation();
            }
        });
